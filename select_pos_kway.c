...
//	int small_n_prevs = n_prevs - 1;
	...
	case MIN:
		while (fins == 0) {
			quals = 0;
			for (int i = 0; i < n_prevs; i++) { quals |= (mm.min <= col[prevs[i][cur_pos[i]]]) * masks[i]; }
			advs = mask_cache;
			for (int i = 0; i < n_prevs; i++) {
				for (int j = 0; j < n_prevs; j++) {
					advs &= (mask_cache - (!(prevs[i][cur_pos[i]] <= prevs[j][cur_pos[j]]) << i));
		...
		if (fins == 1){
			for (int j = cur_pos[1]; j < prev_lens[1]; j++) {
				quals = (mm.min <= col[prevs[1][j]]);
				(*ret)[ret_len] = prevs[1][j];
				ret_len += quals;
			}
		} else if (fins == 2) {
			for (int j = cur_pos[0]; j < prev_lens[0]; j++) {
				quals = (mm.min <= col[prevs[0][j]]);
				(*ret)[ret_len] = prevs[0][j];
				ret_len += quals;
			}
		}
	...
	case MAX:
		while (fins == 0) {
			quals = 0;
			for (int i = 0; i < n_prevs; i++) { quals |= (col[prevs[i][cur_pos[i]]] < mm.max) * masks[i]; }
			advs = mask_cache;
			for (int i = 0; i < n_prevs; i++) {
				for (int j = 0; j < n_prevs; j++) {
					advs &= (mask_cache - (!(prevs[i][cur_pos[i]] <= prevs[j][cur_pos[j]]) << i));
		...
		if (fins == 1){
			for (int j = cur_pos[1]; j < prev_lens[1]; j++) {
				quals = (col[prevs[1][j]] < mm.max);
				(*ret)[ret_len] = prevs[1][j];
				ret_len += quals;
			}
		} else if (fins == 2) {
			for (int j = cur_pos[0]; j < prev_lens[0]; j++) {
				quals = (col[prevs[0][j]] < mm.max);
				(*ret)[ret_len] = prevs[0][j];
				ret_len += quals;
			}
		}
...
int load_selects(minmax_t **select_queries) {
	FILE *fp = fopen("select_queries_kway_pos.txt","r");
	assert(fp != NULL);
	char chr;
	int query_count = 0;
