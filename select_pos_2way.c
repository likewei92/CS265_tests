...
//	int quals[n_prevs];
	...
	case BOTH:
		while ((fin1 == 0) & (fin2 == 0)) {
//			for (int i = 0; i < n_prevs; i++) quals[i] = (mm.min <= col[prevs[i][cur_pos[i]]]) & (col[prevs[i][cur_pos[i]]] < mm.max);
...
//			forward1 = quals[0] & adv1;
//			forward2 = quals[1] & adv2;
			forward1 = qual1 & adv1;
			forward2 = qual2 & adv2;
...
int load_selects(minmax_t **select_queries) {
	FILE *fp = fopen("select_queries_2way_pos.txt","r");
	assert(fp != NULL);
	char chr;
	int query_count = 0;
